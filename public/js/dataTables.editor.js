...
(function (factory) {
  ...
  Editor.Field = function (opts, classes, host) {
    ...
    var template = $(
      '<div class="' +
        ...
        '"></div>' +
        '<div data-dte-e="multi-value" class="' +
        classes.multiValue +
        '">' +
        multiI18n.title +
        '<span data-dte-e="multi-info" class="' +
        classes.multiInfo +
        '">' +
        multiI18n.info +
        "</span>" +
        "</div>" +
        '<div data-dte-e="msg-multi" class="' +
        classes.multiRestore +
        '">' +
        multiI18n.restore +
        "</div>" +
        ...
        classes["msg-error"] +
        '"></div>' +
        '<div data-dte-e="msg-message" class="' +
        classes["msg-message"] +
        '">' +
        opts.message +
        "</div>" +
        '<div data-dte-e="msg-info" class="' +
        classes["msg-info"] +
        '">' +
        opts.fieldInfo +
        "</div>" +
        "</div>" +
        '<div data-dte-e="field-processing" class="' +
        classes.processing +
        '"><span></span></div>' +
        "</div>"
    );
    ...
    this.dom.multi.on("click", function () {
      if (
        that.s.opts.multiEditable &&
        !template.hasClass(classes.disabled) &&
        opts.type !== "readonly"
      ) {
        that.val("");
        that.focus();
  ...
  Editor.Field.prototype = {
    ...
    displayed: function () {
      ...
      return container.parents("body").length &&
    ...
    processing: function (set) {
      if (set === undefined) {
        return this.dom.processing.css("display") === "block";
      }
    ...
    _multiValueCheck: function () {
      ...
      if (
        (different && isMultiValue) ||
        (!isMultiEditable && this.isMultiValue())
      ) {
      ...
      this.dom.multiReturn.css({
        display:
          ids && ids.length > 1 && different && !isMultiValue
            ? "block"
            : "none",
      });
  ...
  (function () {
    ...
    Editor.display.lightbox = $.extend(
      true,
      {},
      Editor.models.displayController,
      {
        ...
        _show: function (callback) {
          ...
          dom.background.on("click.DTED_Lightbox", function (e) {
            e.stopImmediatePropagation();
            self._dte.background();
          });

          $("div.DTED_Lightbox_Content_Wrapper", dom.wrapper).on(
            "click.DTED_Lightbox",
            function (e) {
        ...
        _heightCalc: function () {
          ...
          if (isMobile()) {
            $("div.DTE_Body_Content", dom.wrapper).css(
              "maxHeight",
              "calc(100vh - " + headerFooter + "px)"
            );
          } else {
            // Set the max-height for the form content
            var maxHeight =
              $(window).height() - self.conf.windowPadding * 2 - headerFooter;
        ...
        _hide: function (callback) {
          ...
          $("body").scrollTop(self._scrollTop);
          ...
          // Event handlers
          dom.close.off("click.DTED_Lightbox");
          dom.background.off("click.DTED_Lightbox");
          $("div.DTED_Lightbox_Content_Wrapper", dom.wrapper).off(
            "click.DTED_Lightbox"
          );
  ...
  (function () {
    ...
    Editor.display.envelope = $.extend(
      true,
      {},
      Editor.models.displayController,
      {
        ...
        _show: function (callback) {
          ...
          self._dom.wrapper.style.top =
            $(targetRow).offset().top + targetRow.offsetHeight + "px";
          ...
          $(self._dom.background).on("click.DTED_Envelope", function (e) {
            self._dte.background();
          });

          $("div.DTED_Lightbox_Content_Wrapper", self._dom.wrapper).on(
            "click.DTED_Envelope",
            function (e) {
          ...
          $(window).on("resize.DTED_Envelope", function () {
            self._heightCalc();
          });
        },
        ...
        _hide: function (callback) {
          ...
          $(self._dom.content).animate(
            ...
            600,
            function () {
              $([self._dom.wrapper, self._dom.background]).fadeOut(
                "normal",
                function () {
                  $(this).detach();
                  callback();
          ...
          // Event handlers
          $(self._dom.close).off("click.DTED_Lightbox");
          $(self._dom.background).off("click.DTED_Lightbox");
          $("div.DTED_Lightbox_Content_Wrapper", self._dom.wrapper).off(
            "click.DTED_Lightbox"
          );
          $(window).off("resize.DTED_Lightbox");
        },
  ...
  Editor.prototype.bubble = function (cells, fieldNames, show, opts) {
    ...
    this._edit(cells, editFields, "bubble", opts, function () {
      ...
      var nodes = [];
      that.s.bubbleNodes = nodes.concat.apply(
        nodes,
        _pluck(editFields, "attach")
      );
      ...
      that._closeReg(function (submitComplete) {
        that._animate(pair, { opacity: 0 }, function () {
          if (this === container[0]) {
            pair.detach();
  ...
  Editor.prototype.buttons = function (buttons) {
    ...
    $.each(buttons, function (i, btn) {
      ...
      $("<button></button>", {
        class:
          that.classes.form.button + (btn.className ? " " + btn.className : ""),
      })
        .html(typeof text === "function" ? text(that) : text || "")
  ...
  Editor.prototype.ids = function (includeHash) {
    return $.map(this.s.editFields, function (edit, idSrc) {
      return includeHash === true ? "#" + idSrc : idSrc;
    });
  };
  ...
  Editor.prototype.inline = function (cell, fieldName, opts) {
    ...
    this._edit(cell, editFields, "inline", opts, function () {
      ...
      node
        .find("div." + classes.liner.replace(/ /g, "."))
        .append(field.node())
      ...
      if (opts.buttons) {
        ...
        node
          .find("div." + classes.buttons.replace(/ /g, "."))
          .append(that.dom.buttons);
      }
  ...
  Editor.prototype.remove = function (items, arg1, arg2, arg3, arg4) {
    ...
    this._event(
      "initRemove",
      [_pluck(editFields, "node"), _pluck(editFields, "data"), items],
      function () {
        that._event(
  ...
  Editor.prototype.template = function (set) {
    ...
    this.s.template = set === null ? null : $(set);
  ...
  Editor.prototype.title = function (title) {
    var header = $(this.dom.header).children(
      "div." + this.classes.header.content
    );
  ...
  function __setBasic(inst, opts, type, plural) {
    ...
    if (opts.message === undefined) {
      if (type === "remove") {
        ...
        opts.message =
          plural !== 1 ? confirm._.replace(/%d/, plural) : confirm["1"];
      } else {
        opts.message = "";
      }
    }
  ...
  Editor.upload = function (
    ...
    reader.onload = function (e) {
      ...
      var preRet = editor._event("preUpload", [
        conf.name,
        files[counter],
        data,
      ]);
  ...
  Editor.prototype._constructor = function (init) {
    ...
    this.dom = {
      ...
      formError: $(
        '<div data-dte-e="form_error" class="' + classes.form.error + '"></div>'
      )[0],
      formInfo: $(
        '<div data-dte-e="form_info" class="' + classes.form.info + '"></div>'
      )[0],
      header: $(
        '<div data-dte-e="head" class="' +
      ...
      buttons: $(
        '<div data-dte-e="form_buttons" class="' +
    ...
    $(document)
      ...
      .on("xhr.dt.dte" + this.s.unique, function (e, settings, json) {
        ...
        if (
          json &&
          that.s.table &&
          settings.nTable === $(that.s.table).get(0)
        ) {
  ...
  Editor.prototype._actionClass = function () {
    ...
    wrapper.removeClass(
      [classesActions.create, classesActions.edit, classesActions.remove].join(
        " "
      )
    );
  ...
  Editor.prototype._ajax = function (data, success, error, submitParams) {
    ...
    if (opts.data) {
      ...
      data = isFn && newData ? newData : $.extend(true, data, newData);
    }
    ...
    if (
      opts.type === "DELETE" &&
      (opts.deleteBody === undefined || opts.deleteBody === true)
    ) {
  ...
  Editor.prototype._animate = function (target, style, time, callback) {
    if ($.fn.animate) {
      target.stop().animate(style, time, callback);
    } else {
      target.css(style);
   ...
   * @private
   */
  Editor.prototype._clearDynamicInfo = function (errorsOnly) {
  ...
  Editor.prototype._displayReorder = function (includeFields) {
    ...
    $.each(order, function (i, fieldOrName) {
      var name =
        fieldOrName instanceof Editor.Field ? fieldOrName.name() : fieldOrName;

      if (that._weakInArray(name, includeFields) !== -1) {
        if (template && mode === "main") {
          template
            .find('editor-field[name="' + name + '"]')
            .after(fields[name].node());

          template
            .find('[data-editor-template="' + name + '"]')
            .append(fields[name].node());
        } else {
   ...
   * @private
   */
  Editor.prototype._edit = function (
    items,
    editFields,
    type,
    formOptions,
  ...
  Editor.prototype._postopen = function (type, immediate) {
    ...
    if (focusCapture && (type === "main" || type === "bubble")) {
      $("body").on("focus.editor-focus", function () {
        if (
          $(document.activeElement).parents(".DTE").length === 0 &&
  ...
  Editor.prototype._preopen = function (type) {
    ...
    if (this._event("preOpen", [type, this.s.action]) === false) {
      ...
      if (
        (this.s.mode === "inline" || this.s.mode === "bubble") &&
        this.s.closeIcb
      ) {
        this.s.closeIcb();
      }
   ...
   * @private
   */
  Editor.prototype._submit = function (
    successCallback,
    errorCallback,
    formatdata,
    hide
      ...
      $.each(editFields, function (idSrc, edit) {
        ...
        $.each(fields, function (name, field) {
          if (edit.fields[name] && field.submittable()) {
            ...
            var manyBuilder =
              Array.isArray(value) && name.indexOf("[]") !== -1
            ...
            if (
              action === "edit" &&
              (!editData[name] || !field.compare(value, editData[name][idSrc]))
            ) {
    ...
    this._event("preSubmit", [submitParams, action], function (result) {
      if (result === false) {
        ...
        var submitWire =
          that.s.ajax || that.s.ajaxUrl ? that._ajax : that._submitTable;
   ...
   * @private
   */
  Editor.prototype._submitTable = function (
    data,
    success,
    error,
    submitParams
   ...
   * @private
   */
  Editor.prototype._submitSuccess = function (
    json,
    notGood,
    submitParams,
    submitParamsLocal,
   ...
   * @private
   */
  Editor.prototype._submitError = function (
    xhr,
    err,
    thrown,
    errorCallback,
    ...
    this._event(
      ["submitError", "submitComplete"],
      [xhr, err, thrown, submitParams]
    );
  };
  ...
  Editor.prototype._tidy = function (fn) {
    var that = this;
    var dt = this.s.table ? new $.fn.dataTable.Api(this.s.table) : null;
  ...
  (function () {
    ...
    var __dtCellSelector = function (
      ...
      dt.cells(identifier)
        ...
        .each(function (idx) {
          ...
          var idSrc = idFn(data);
          var fields =
            forceFields || __dtFieldsFromIdx(dt, allFields, idx.column);
          var isNode =
            (typeof identifier === "object" && identifier.nodeName) ||
    ...
    __dataSources.dataTable = {
      ...
      commit: function (action, identifier, data, store) {
        ...
        if (drawType !== "none") {
          ...
          // Rebuild searchpanes
          if (
            typeof dt.searchPanes === "function" &&
            !dt.settings()[0].oFeatures.bServerSide
          ) {
    ...
    function __html_id(identifier) {
      ...
      if (identifier !== "keyless") {
        ...
        if (context.length === 0) {
          throw (
            "Could not find an element with `data-editor-id` or `id` of: " +
            identifier
          );
        }
      }
  ...
  (function () {
    if (DataTable.TableTools) {
      ...
      ttButtons.editor_edit = $.extend(
        true,
        ttButtons.select_single,
        ttButtonBase,
        {
          formButtons: [
            {
      ...
      ttButtons.editor_remove = $.extend(true, ttButtons.select, ttButtonBase, {
        ...
        fnClick: function (button, config) {
          ...
          var buttons = config.formButtons;
          var question =
            typeof i18nRemove.confirm === "string"
              ? i18nRemove.confirm
              : i18nRemove.confirm[rows.length]
              ? i18nRemove.confirm[rows.length]
              : i18nRemove.confirm._;
  ...
  $.extend(Editor.DateTime.prototype, {
      ...
      this.dom.container
        .on("change", "select", function () {
          ...
          if (select.hasClass(classPrefix + "-month")) {
            ...
            select.hasClass(classPrefix + "-hours") ||
            select.hasClass(classPrefix + "-ampm")
          ) {
    ...
    _htmlMonth: function (year, month) {
      ...
      for (var i = 0, r = 0; i < cells; i++) {
        ...
        if (
          ...
          $.inArray(day.getUTCDay(), disableDays) !== -1
        ) {
          disabled = true;
        } else if (
    ...
    _position: function () {
      ...
      if (
        offset.top + inputHeight + calHeight - scrollTop >
        $(window).height()
      ) {
    ...
    _show: function () {
      ...
      setTimeout(function () {
        $("body").on("click." + namespace, function (e) {
          ...
          if (
            !parents.filter(that.dom.container).length &&
            e.target !== that.dom.input[0]
          ) {
            that._hide();
          }
        });
    ...
    _writeOutput: function (focus) {
      ...
      var out = window.moment
        ? window.moment
            .utc(date, undefined, this.c.momentLocale, this.c.momentStrict)
      ...
      this.dom.input.val(out).trigger("change", { write: date });
  ...
  (function () {
    ...
    function _commonUpload(editor, conf, dropCallback, multiple) {
      ...
      if (window.FileReader && conf.dragDrop !== false) {
        container
          .find("div.drop span")
          .text(conf.dragDropText || "Drag and drop a file here to upload");
    ...
    fieldTypes.textarea = $.extend(true, {}, baseFieldType, {
      create: function (conf) {
        conf._input = $("<textarea></textarea>").attr(
          $.extend(
            {
              id: Editor.safeId(conf.id),
            },
        ...
        if (!append) {
          ...
          if (conf.placeholder !== undefined) {
            var placeholderValue =
              conf.placeholderValue !== undefined ? conf.placeholderValue : "";

            countOffset += 1;
            elOpts[0] = new Option(conf.placeholder, placeholderValue);

            var disabled =
              conf.placeholderDisabled !== undefined
    ...
    fieldTypes.radio = $.extend(true, {}, baseFieldType, {
      ...
      update: function (conf, options, append) {
        ...
        radio.set(
          conf,
          inputs.filter('[value="' + currVal + '"]').length
            ? currVal
    ...
    fieldTypes.date = $.extend(true, {}, baseFieldType, {
      ...
      owns: function (conf, node) {
        return $(node).parents("div.ui-datepicker").length ||
          $(node).parents("div.ui-datepicker-header").length
          ? true
          : false;
      },
    });
    ...
    fieldTypes.datetime = $.extend(true, {}, baseFieldType, {
      create: function (conf) {
        ...
        conf._picker = new Editor.DateTime(
          conf._input,
          $.extend(
            {
      ...
      set: function (conf, val) {
        ...
        if (conf.display) {
          ...
          if (val.length) {
            ...
            rendered.append(
              "<span>" + (conf.noFileText || "No files") + "</span>"
            );
          }
        }
